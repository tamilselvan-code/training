	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (pname != null) {
			...
			try {
				 ...
				 * 
				 * ResultSet rs = d.validate1(sql1); //set status in session if(rs.next()) {
				 * String s = rs.getString(15); System.out.println("Status = "+s); HttpSession
				 * session1 = request.getSession(); session1.setAttribute("s1",s); }
				 */
				if (i == 1) {

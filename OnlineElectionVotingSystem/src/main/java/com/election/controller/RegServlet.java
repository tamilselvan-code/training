...
public class RegServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/// in dob validation we nly focus on YEAR(yyyy) part validation.....ignore
		/// month & day
		System.out.println(dob);
		String s = dob.substring(0, 4); // 0-> starting index & 4-> character counting (yyyy format in dob so 4
										// character thts y 4)
		Date d = new Date();
		...
		if (pass.equals(cpass)) {
			...
			if (a > 1950 && (b - a) >= 18) {
				if (captcha.equals(code)) {
					...
					m.setPass(pass);
					m.setDob(dob);
					String sql = "insert into register(FIRST_NAME,MIDDLE_NAME,LAST_NAME, GENDER,CONTACT,EMAIL,ADDRESS, COUNTRY,STATE,AADHAR, ELECTION, PASSWORD,DOB,USERNAME) values('"
							+ fn + "', '" + mn + "', '" + ln + "','" + gend + "','" + cont + "','" + email + "','" + add
							+ "','" + country + "','" + state + "','" + aadhar + "', '" + election + "','" + pass
							+ "', '" + dob + "', '" + username + "')";
